...
local term_utils = require("harpoon.utils.term")
...
local function create_terminal()
    ...
    local out, ret, _ = term_utils.get_os_command_output({
        "tmux",
        "new-window",
        "-P",
        "-F",
...
local function terminal_exists(window_id)
    ...
    local window_list, _, _ = term_utils.get_os_command_output({
        "tmux",
        "list-windows",
    }, vim.loop.cwd())
    ...
    for _, line in pairs(window_list) do
        local window_info = string_utils.split_string(line, "@")[1]
...
M.gotoTerminal = function(idx)
    ...
    term_utils.get_os_command_output({
        "tmux",
        "select-window",
        "-t",
...
M.sendCommand = function(idx, cmd, ...)
    ...
    if cmd then
        ...
        local _, _, _ = term_utils.get_os_command_output({
            "tmux",
            "send-keys",
            "-t",
            "%" .. window_handle.window_id,
